...
例如，用户账户中有 `100` 余额，可以取出 `20` 生成一张面值为 `20` 的兑换码。生成成功后，用户余额变成 `80`，兑换码可以在 sub2api 的兑换页面使用。
...
5. 工具只采用 profile 返回的真实用户 ID、用户名和余额，不信任 URL 中的 `user_id`。
6. 验证成功后，工具把 `用户 JWT + 用户 ID` 加密到 HttpOnly Cookie。
7. 用户提交兑换金额后，工具再次读取实时 profile，并签发绑定用户、金额和操作 ID 的短期操作令牌。
8. 工具使用只保存在服务端的管理员 API Key 生成兑换码，再对已经验证的用户 ID 扣减余额。
9. 页面显示兑换码，并重新查询实时余额。

后续的 `/api/me`、准备兑换和执行兑换都会重新向 sub2api 验证用户状态。浏览器提交的用户名、用户 ID 或余额都不会被当作可信数据。
...
2. **生成兑换码和扣减余额不是同一个数据库事务。** 两次管理员请求之间如果断电、超时或进程退出，可能留下需要人工核对的兑换码。
...
如果把 `PORT` 改成 `4000`，Docker 映射必须相应改为：
